...
///
/// Defaults to `false`.
@property (nonatomic, readonly, getter=isPageGrabberEnabled) BOOL pageGrabberEnabled PSPDF_DEPRECATED_IOS(10.5, "Deprecated in PSPDFKit 10.5 for iOS because the scroll indicator from `UIScrollView` supports dragging on iOS 13 and later.");
...
///
/// This applies to all ways of adding an annotation, whether using an annotation tool
/// provided by the PSPDFKit UI or adding an annotation programatically.
///
/// Not supported for text markup, free text, note, polygon, or poly-line annotations.
///
/// Enabling this option results in the `drawCreateMode` effectively acting as `.separate`
/// because selecting a drawing forces subsequent strokes to be placed in a new annotation.
///
/// For some annotation types such as stamps, the menu is always shown after creation (even if this
/// property is `false`). The menu is also always shown when pasting any annotation or when adding
/// annotation from the saved annotations list.
@property (nonatomic, readonly) BOOL showAnnotationMenuAfterCreation PSPDF_DEPRECATED_IOS(10.4, "Observe the `PSPDFAnnotationsAdded` notification and programatically select the new annotation and show the menu.");
...
@property (nonatomic) BOOL showAnnotationMenuAfterCreation PSPDF_DEPRECATED_IOS(10.4, "Observe the `PSPDFAnnotationsAdded` notification and programatically select the new annotation and show the menu.");
